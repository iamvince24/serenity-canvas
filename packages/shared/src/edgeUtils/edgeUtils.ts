...
export function computeEdgePath(
  edge: Edge,
  nodes: Record<string, CanvasNode>,
): { d: string; labelPos: Point | null; route: EdgeRoute | null } {
  const route = getEdgeRoute(edge, nodes);
  if (!route) {
    return { d: "", labelPos: null, route: null };
  }

  return {
    d: `M ${route.start.x} ${route.start.y} C ${route.cp1.x} ${route.cp1.y}, ${route.cp2.x} ${route.cp2.y}, ${route.end.x} ${route.end.y}`,
    labelPos: route.midpoint,
    route,
  };
}
